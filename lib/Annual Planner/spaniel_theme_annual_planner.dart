...
class SpanielThemeCalendarApp extends StatefulWidget {
  const SpanielThemeCalendarApp({super.key});

  @override
  State<SpanielThemeCalendarApp> createState() =>
      _SpanielThemeCalendarAppState();
}
...
class _SpanielThemeCalendarAppState extends State<SpanielThemeCalendarApp> {
  ...
  Future<void> _saveDates() async {
    try {
      ...
      await prefs.setString('spaniel_calendar_theme_2025', jsonData);
    } catch (e) {
  ...
  Future<void> _loadSavedDates() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final String? savedData = prefs.getString('spaniel_calendar_theme_2025');
  ...
  Future<void> _takeScreenshotAndShare() async {
    try {
      ...
      final imagePath = '${directory.path}/spaniel_theme_calendar_2025.png';
      final imageFile = File(imagePath);
      await imageFile.writeAsBytes(image);

      await Share.shareXFiles([XFile(imagePath)],
          text: 'My Spaniel Theme Calendar 2025');
    } catch (e) {
        ...
        children: [
          Flexible(
            ...
            child: ClipRRect(
              ...
              child: Image.asset(
                'assets/spaniel_calender/cocker${monthIndex + 1}.png',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Spaniel Theme Calendar 2025'),
      ),
      body: Column(
        children: [
          Expanded(
